...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        super().__init__()
        if isinstance(pt_seq_len, tuple):
            pt_h, pt_w = pt_seq_len
        else:
            pt_h = pt_w = pt_seq_len
        if ft_seq_len is None:
            ft_h, ft_w = pt_h, pt_w
        elif isinstance(ft_seq_len, tuple):
        ...
        t_h = torch.arange(ft_h) / max(ft_h, 1) * pt_h
        t_w = torch.arange(ft_w) / max(ft_w, 1) * pt_w
        ...
        if num_cls_token > 0:
            ...
            device = cos_img.device
            cos_pad = torch.ones(num_cls_token, D, dtype=cos_img.dtype, device=device)
            sin_pad = torch.zeros(num_cls_token, D, dtype=sin_img.dtype, device=device)
...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0):
    ...
    return:
    pos_embed: [H*W, embed_dim] or [extra_tokens+H*W, embed_dim] (w/ or w/o cls_token)
    """
