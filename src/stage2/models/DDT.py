...
class DiTwDDTHead(nn.Module):
    def __init__(
        ...
        if self.rope_mode not in {"2d", "1d"}:
            raise ValueError(f"Unsupported rope_mode '{rope_mode}'. Supported: 2d, 1d.")

        def _to_hw(value, name: str) -> tuple[int, int]:
            if isinstance(value, Iterable) and not isinstance(value, (str, bytes)):
                ...
                if seq is not None:
                    if len(seq) != 2:
                        raise ValueError(f"{name} must be int or [H, W], got {value}.")
                    return int(seq[0]), int(seq[1])
            v = int(value)
            return v, v

        input_hw = _to_hw(input_size, "input_size")
        # analyze patch size
        ...
        s_input_size = input_hw
        s_patch_size = self.s_patch_size
        x_input_size = input_hw
        ...
        if self.use_rope:
            ...
            if self.rope_mode == "1d":
                self.enc_feat_rope = SequenceRotaryEmbeddingFast(
                    dim=self.encoder_hidden_size // enc_num_heads,
                    pt_seq_len=int(self.s_embedder.num_patches),
                )
            else:
                s_grid_h, s_grid_w = self.s_embedder.grid_size
                if s_grid_h != s_grid_w:
                    raise ValueError(
                        "rope_mode=2d requires square token grid for stage2 encoder. "
                        f"Got grid_size={self.s_embedder.grid_size}. Use rope_mode=1d."
                    )
                self.enc_feat_rope = VisionRotaryEmbeddingFast(
                    dim=enc_half_head_dim,
                    pt_seq_len=int(s_grid_h),
                )
            dec_half_head_dim = self.decoder_hidden_size // dec_num_heads // 2
            if self.rope_mode == "1d":
                self.dec_feat_rope = SequenceRotaryEmbeddingFast(
                    dim=self.decoder_hidden_size // dec_num_heads,
                    pt_seq_len=int(self.x_embedder.num_patches),
                )
            else:
                x_grid_h, x_grid_w = self.x_embedder.grid_size
                if x_grid_h != x_grid_w:
                    raise ValueError(
                        "rope_mode=2d requires square token grid for stage2 decoder. "
                        f"Got grid_size={self.x_embedder.grid_size}. Use rope_mode=1d."
                    )
                self.dec_feat_rope = VisionRotaryEmbeddingFast(
                    dim=dec_half_head_dim,
                    pt_seq_len=int(x_grid_h),
                )
        else:
            self.feat_rope = None
    ...
    def initialize_weights(self, xavier_uniform_init: bool = False):
        ...
        if self.use_pos_embed:
            ...
            grid_h, grid_w = self.s_embedder.grid_size
            if grid_h == grid_w:
                pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], int(grid_h))
            else:
    ...
    def unpatchify(self, x):
        ...
        if h * w != x.shape[1]:
            raise ValueError(
                f"Token count mismatch in unpatchify: expected {h*w}, got {x.shape[1]}."
            )
