...
def get_2d_sincos_pos_embed_hw(embed_dim, grid_h, grid_w, cls_token=False, extra_tokens=0):
    """
    2D sin-cos embedding for rectangular grids.
    Returns [grid_h*grid_w, embed_dim] (or with extra cls tokens prepended).
    """
    grid_h_arr = np.arange(grid_h, dtype=np.float32)
    grid_w_arr = np.arange(grid_w, dtype=np.float32)
    grid = np.meshgrid(grid_w_arr, grid_h_arr)  # w first
...
class SequenceRotaryEmbeddingFast(nn.Module):
    """
    1D rotary embedding for sequence-like token layouts.
    """
    ...
    def forward(self, t):
        ...
        if Lt % L != 0:
            raise ValueError(f"Token length {Lt} is not divisible by rope base length {L}.")
        repeat = Lt // L
        freqs_cos, freqs_sin = self.freqs_cos, self.freqs_sin
        if repeat != 1:
            freqs_cos = freqs_cos.repeat_interleave(repeat, dim=0)
