...
class ICPlan:
    ...
    def get_noise_from_x1(self, x1, x, t, eps: float = 1e-8):
        """Recover endpoint noise x0 from x_t and predicted clean endpoint x1."""
        t = expand_t_like_x(t, x)
        alpha_t, _ = self.compute_alpha_t(t)
        sigma_t, _ = self.compute_sigma_t(t)
        sigma_safe = sigma_t.abs().clamp_min(eps)
        noise = (x - alpha_t * x1) / sigma_safe
        return noise

    def get_velocity_from_x1(self, x1, x, t, eps: float = 1e-8):
        """Convert x1 prediction to velocity parameterization u_t."""
        t = expand_t_like_x(t, x)
        _, d_alpha_t = self.compute_alpha_t(t)
        _, d_sigma_t = self.compute_sigma_t(t)
        noise = self.get_noise_from_x1(x1, x, t, eps=eps)
        velocity = d_alpha_t * x1 + d_sigma_t * noise
        return velocity

    def get_score_from_x1(self, x1, x, t, eps: float = 1e-8):
        """Convert x1 prediction to score of x_t."""
        t = expand_t_like_x(t, x)
        sigma_t, _ = self.compute_sigma_t(t)
        sigma_safe = sigma_t.abs().clamp_min(eps)
        noise = self.get_noise_from_x1(x1, x, t, eps=eps)
        score = -noise / sigma_safe
        return score
