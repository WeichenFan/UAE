...
class ModelType(enum.Enum):
    ...
    X_START = enum.auto()  # the model predicts clean endpoint x1
...
class Transport:
    ...
    def check_interval(
        ...
        *, 
        ...
        # Defensive fallback for legacy configs/overrides that may leave eps unset.
        if eps is None:
            eps = 0.0
    ...
    def training_losses(
        ...
        if self.model_type == ModelType.VELOCITY:
        ...
        elif self.model_type == ModelType.X_START and self.loss_type in [WeightType.VELOCITY]:
            pred_velocity = self.path_sampler.get_velocity_from_x1(model_output, xt, t)
            terms['pred_velocity'] = pred_velocity
            ...
            elif self.model_type == ModelType.X_START:
                terms['loss'] = mean_flat(weight * ((model_output - x1) ** 2))
            else:
                raise NotImplementedError(f"Unsupported model_type for training loss: {self.model_type}")
    ...
    def get_drift(
        ...
        def xstart_ode(x, t, model, **model_kwargs):
            x1_pred = model(x, t, **model_kwargs)
            return self.path_sampler.get_velocity_from_x1(x1_pred, x, t)
    ...
    def get_score(
        ...
        if self.model_type == ModelType.NOISE:
            ...
            score_fn = lambda x, t, model, **kwargs: self.path_sampler.get_score_from_x1(model(x, t, **kwargs), x, t)
        else:
            raise NotImplementedError()
